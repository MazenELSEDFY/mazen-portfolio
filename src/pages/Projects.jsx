...
const ProjectCard = ({ p }) => (
  <Card>
    <div className="flex items-baseline justify-between gap-2">
      ...
      {p.timeframe && (
        <span className="text-xs text-brand.orange">{p.timeframe}</span>
      )}
    </div>
    {p.image && (
      <img
      ...
      <Section title="All Projects">
        {list.length === 0 ? (
          <div className="text-brand.blue/80">
            No projects yet. Add items to{" "}
            <code className="rounded px-1 py-0.5 bg-brand.orange/10 border border-brand.blue/10">
